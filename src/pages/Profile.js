  ...
  const updateHandler = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyC0YxGULLqQAQ8zCL5hH28a6TutuM4g3GE",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyC0YxGULLqQAQ8zCL5hH28a6TutuM4g3GE",
          {
            method: "POST",
            body: JSON.stringify({
              idToken: token,
    ...
    <>
      <div className={classes.topText}>
        ...
        <div className={classes.profileInfo}>
          Your profile is 60% complete. AComplete Profile has a higher chance of
      ...
      <div className={classes["profile-form-container"]}>
        ...
        {!loading && (
          <button className={classes["update-button"]} onClick={updateHandler}>
            Update
          </button>
        )}
